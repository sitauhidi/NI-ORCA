...
class ORCA_FHM_VERTEX : public orca::ORCA {
	public:
...
class ORCA_UOM_VERTEX : public orca::ORCA {
	public:
	void orbit_count() override {
		std::chrono::time_point<std::chrono::system_clock> start, end;
		start = std::chrono::system_clock::now();
...
class ORCA_ARRAY_VERTEX : public orca::ORCA {
	public:
	void orbit_count() override {
    	std::chrono::time_point<std::chrono::system_clock> start, end;
  		start = std::chrono::system_clock::now();
		...
		#pragma omp parallel for schedule(runtime)
		for (int x = 0; x < n; x++) {
			int *common = (int*)calloc(n, sizeof(int)); // Thread-private array
			int *common_list = (int*)malloc(n * sizeof(int)), nc = 0;
